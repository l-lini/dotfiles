  ...
  wayland.windowManager.sway =
    let
      commands = rec {
        ...
        move =
          delta: fetchOneCommand: fetchAllCommand:
          "bash -c '"
          + "mapfile -t arr < <("
          + wrapCommand fetchAllCommand
          + "); "
          + "l=$\{#arr[@]}; "
          + "for ((i=0; i<l; i++)); do "
          + "  if [[ \"$\{arr[i]}\" == $("
          + wrapCommand fetchOneCommand
          + ") ]]; then "
          + "    prev=$(((i + "
          + toString delta
          + " + l) % l)); "
        ...
        wpctl = rec {
          ...
          set = rec {
            volume =
              idCommand: delta: max:
              "wpctl set-volume ${wrapCommand idCommand} ${delta} -l ${max}";
          ...
          // (
            ...
            with keys;
            {
              ...
              "${modifier}+XF86AudioRaiseVolume" =
                "exec ${source.volumeUp} && ${notification.source.volumeAndMuted}";
              "${modifier}+XF86AudioLowerVolume" =
                "exec ${source.volumeDown} && ${notification.source.volumeAndMuted}";
